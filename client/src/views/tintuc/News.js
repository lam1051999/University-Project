...
import { fetchNews , deleteNews } from '../../redux/action/action';
import {connect} from 'react-redux';
import {withRouter} from 'react-router';
import {Spinner , Carousel , Button} from 'react-bootstrap';
        ...
        return(
            ...
            this.props.nw.map((en,i) => 
                            <Carousel.Item key={i}>
                                ...
                                <Button style={{position : 'absolute' , top : '80px' , right : '100px'}} variant="danger" onClick={() => 
                                {
                                    if(window.confirm('Bạn có chắc chắn muốn xóa tin tức này ra khỏi danh sách tin tức không?'))
                                        this.props.dispatch(deleteNews(en.ID))
                                }} >Xóa</Button>
                            </Carousel.Item>
            ):<Carousel.Item>
            <img
